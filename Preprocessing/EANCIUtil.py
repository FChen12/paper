...
from Dataset import EANCI
import FileUtil, os

def rename_eanci_code_files_and_solution_matrix():
    tok = JavaCodeASTTokenizer(EANCI(), WordTokenizer(EANCI(), True))
    code_files = FileUtil.get_files_in_directory(EANCI().code_folder(), True)
    code_file_to_class_name_map = {}
    old_sol_matrix = FileUtil.read_xml_format_solution_matrix(EANCI().folder() / "answer_req_code.xml")
    for code_file in code_files:
        ...
        os.rename(code_file, EANCI().code_folder() / new_file_name)
    ...
    FileUtil.write_file(EANCI().EANCI_SOLUTION_MATRIX_PATH, "\n".join(renamed_solution_links))
